...
在常规的 Python 代码中，方括号 [] 代表列表（可变），圆括号 () 代表元组（不可变），它们是完全不同的数据结构。  
但是在match/case中，它们都代表序列  
比如case[a,b]  
case(a,b)  
case a,b  


通常来说，凡是能用 for 循环遍历、能用 len() 求长度、能用 [0] 取索引的东西，都算序列（继承自 collections.abc.Sequence）。match/case 的序列模式支持绝大多数这样的对象（如 list, tuple, deque 等）。唯独例外的是：str（字符串）、bytes 和 bytearray。Python 设计者故意在模式匹配中禁止了把字符串当作序列来拆包。
